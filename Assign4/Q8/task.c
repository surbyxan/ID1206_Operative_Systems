...
int main() {
    ...
    map_address = mmap(NULL, FILE_SIZE,
    ...
    if (pid == 0) {
        // Child process
        memcpy(map_address, "01234", 5);
        msync(map_address, FILE_SIZE, MS_SYNC);
        char buffer[5];
        memcpy(buffer, map_address+4096, 5);
        printf("Child Process ( PID: %d ); read from mmaped [4096]: %s\n",
            getpid(), buffer);
    } else {
        // Parent process
        memcpy(map_address+4096,"56789", 5);
        msync(map_address, FILE_SIZE, MS_SYNC);
        wait(NULL);
        char buffer[5];
        memcpy(buffer, map_address, 5);
